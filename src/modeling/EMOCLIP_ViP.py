...
class CLIPTextEmbeddings(nn.Module):
    ...
    def forward(
        ...
        if emotions is not None:
            ...
            emotion_embeds = selected_emotion_embeds.sum(1) / safe_divisor            
...
class CLIPPreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module):
        ...
        elif isinstance(module, EMOCLIPModel):
            nn.init.normal_(
...
class EMOCLIPModel(CLIPPreTrainedModel):
    config_class = CLIPConfig
    ...
    def get_text_features(
        ...
        Examples:
        ```python
        >>> from transformers import CLIPTokenizer, EMOCLIPModel
        >>> model = EMOCLIPModel.from_pretrained("openai/clip-vit-base-patch32")
        >>> tokenizer = CLIPTokenizer.from_pretrained("openai/clip-vit-base-patch32")
        >>> inputs = tokenizer(["a photo of a cat", "a photo of a dog"], padding=True, return_tensors="pt")
    ...
    def get_image_features(
        ...
        >>> from PIL import Image
        >>> import requests
        >>> from transformers import CLIPProcessor, EMOCLIPModel
        >>> model = EMOCLIPModel.from_pretrained("openai/clip-vit-base-patch32")
        >>> processor = CLIPProcessor.from_pretrained("openai/clip-vit-base-patch32")
        >>> url = "http://images.cocodataset.org/val2017/000000039769.jpg"
        >>> image = Image.open(requests.get(url, stream=True).raw)
