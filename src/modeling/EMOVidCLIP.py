...
from src.modeling.EMOCLIP_ViP import EMOCLIPModel, clip_loss
from src.modeling.CLIP import CLIPModel as CLIP              
                
class EMOVidCLIP(nn.Module):
    def __init__(self, args):
        super(EMOVidCLIP, self).__init__()
        ...
        if args.clip_weights:
            if self.vision_additional_config.type == "ViP":
                self.clipmodel = EMOCLIPModel.from_pretrained(args.clip_weights, config=clipconfig)
            else:
                self.clipmodel = CLIP.from_pretrained(args.clip_weights, config=clipconfig)
        else:
            if self.vision_additional_config.type == "ViP":
                self.clipmodel = EMOCLIPModel(clipconfig)
            else:
    ...
    def forward(self, video, text_input_ids, text_input_mask, emotions, \
