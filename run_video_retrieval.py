...
from src.modeling.EMOVidCLIP import EMOVidCLIP

from src.datasets.dataset_video_retrieval import (
    HDVILAVideoRetrievalDataset_embed, VideoRetrievalCollator_embed)
...
def mk_video_ret_dataloader_embed(dataset_name, vis_format, anno_path, vis_dir, cfg, tokenizer, mode):
    """"""
    is_train = mode == "train"
    dataset = HDVILAVideoRetrievalDataset_embed(
        cfg=cfg,
        vis_dir=vis_dir,
        anno_path=anno_path,
        vis_format=vis_format,
    ...
    sampler = DistributedSampler(
        dataset, num_replicas=hvd.size(), rank=hvd.rank(),
        shuffle=is_train)
    vret_collator = VideoRetrievalCollator_embed(
        tokenizer=tokenizer, max_length=cfg.max_txt_len, is_train=is_train)
    dataloader = DataLoader(dataset,
                            batch_size=batch_size,
                            shuffle=False,
...
def setup_dataloaders_embed(cfg, tokenizer):
    LOGGER.info("Init. train_loader and val_loader...")

    db = cfg.train_datasets
    train_loader = mk_video_ret_dataloader_embed(
        dataset_name=db.name, vis_format=db.vis_format,
        anno_path=db.txt, vis_dir=db.vis,
        cfg=cfg, tokenizer=tokenizer, mode="train"
    )
    ...
    val_loaders = {}
    for db in cfg.inference_datasets:
        val_loaders[db.name] = mk_video_ret_dataloader_embed(
            dataset_name=db.name, vis_format=db.vis_format,
            anno_path=db.txt, vis_dir=db.vis,
            cfg=cfg, tokenizer=tokenizer, mode="val"
        )
    ...
    inference_loaders = {}
    for db in cfg.inference_datasets:
        inference_loaders[db.name] = mk_video_ret_dataloader_embed(
            dataset_name=db.name, vis_format=db.vis_format,
            anno_path=db.txt, vis_dir=db.vis,
            cfg=cfg, tokenizer=tokenizer, mode="test"
        )
...
def setup_model_embed(cfg, device=None):
    LOGGER.info("Setup model...")
    
    model = EMOVidCLIP(cfg)
...
def start_training():
    ...
    if hvd.rank() != 0:
        LOGGER.disabled = True
    if cfg.is_embed:
        model = setup_model_embed(cfg, device=device)
    else:
    ...
    if cfg.is_embed:
        train_loader, val_loaders, inference_loaders = setup_dataloaders_embed(cfg, tokenizer)
    else:
